...
<head>
    ...
    <title><?= esc($page_title ?? 'Edit User') ?></title>
    <!-- Font Awesome CSS -->
...
<body>
    <div class="form-container">
        <div class="form-header">
            <h1><i class="fas fa-user-edit"></i> <?= esc($page_title ?? 'Επεξεργασία Χρήστη') ?></h1>
            <p>Επεξεργαστείτε τα στοιχεία του χρήστη</p>
        </div>
        ...
        <?php if (isset($message) && $message === 'User updated successfully'): ?>
            <div class="success-message">
                <i class="fas fa-check-circle"></i> Ο χρήστης ενημερώθηκε επιτυχώς!
            </div>
        <?php endif; ?>
        ...
        <form method="POST" action="<?= base_url('user/edit/' . $user->id) ?>" id="editUserForm">
            ...
            <div class="form-group">
                ...
                <input 
                    ...
                    id="password" 
                    name="password" 
                    placeholder="Αφήστε κενό για να διατηρήσετε τον τρέχοντα κωδικό"
                    minlength="8"
                >
                <div class="help-text">
                    <span class="optional-text">Προαιρετικό:</span> Ελάχιστο 8 χαρακτήρες. Αφήστε κενό για να διατηρήσετε τον τρέχοντα κωδικό.
                </div>
                <div class="error-message"></div>
            </div>
            ...
            <div class="form-group">
                <label for="password_confirm">
                    <i class="fas fa-lock"></i> Επιβεβαίωση Κωδικού
                </label>
                <input 
                    type="password" 
                    id="password_confirm" 
                    name="password_confirm" 
                    placeholder="Επαναλάβετε τον νέο κωδικό πρόσβασης"
                    minlength="8"
                >
                <div class="help-text">
                    <span class="optional-text">Προαιρετικό:</span> Συμπληρώστε μόνο αν αλλάζετε τον κωδικό.
                </div>
                <div class="error-message"></div>
            </div>
        ...
        <?php else: ?>
            <div class="error-message" style="display: block; background-color: #f8d7da; color: #721c24; padding: 12px 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #f5c6cb;">
                <i class="fas fa-exclamation-triangle"></i> Δεν βρέθηκε ο χρήστης.
            </div>
            <div class="btn-group">
                <a href="<?= base_url('users') ?>" class="btn btn-secondary" style="text-decoration: none;">
        ...
        document.getElementById('editUserForm')?.addEventListener('submit', function(e) {
            ...
            if (password || passwordConfirm) {
                ...
                if (password && password.length < 8) {
                    ...
                    const errorMsg = passwordField.querySelector('.error-message');
                    errorMsg.textContent = 'Ο κωδικός πρέπει να έχει τουλάχιστον 8 χαρακτήρες';
                    return false;
                }
            }
        });
